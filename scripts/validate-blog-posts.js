...
function parseFrontmatter(content) {
  ...
  if (!content.startsWith('---')) {
    return { error: 'File does not start with frontmatter delimiter (---)', frontmatter: null, body: null };
  }
  ...
  if (frontmatterEndIndex === -1) {
    return { error: 'Frontmatter closing delimiter (---) not found', frontmatter: null, body: null };
  }
...
function validateFile(filePath) {
  ...
  if (!dateRegex.test(frontmatter.date)) {
    error(`  Date format invalid. Expected YYYY-MM-DD, got: ${frontmatter.date}`);
    return false;
  }
